...
class RapportRepository(private val clock: Clock) {
    ...
    fun lagreRapport(tx: TransactionalSession, rapport: UlagretRapport): Rapport =
        queryOf(
                """
                INSERT INTO rapport.rapport(bestilling_id, orgnr, type, dato_valutert, bankkonto, navn, antall_rader, antall_underenheter, antall_personer)
                VALUES (:bestilling_id, :orgnr, CAST(:type AS rapport.rapport_type), :dato_valutert, :bankkonto, :navn, :antall_rader, :antall_underenheter, :antall_personer)
                RETURNING *
                """
                    .trimIndent(),
                mapOf(
    ...
    fun finnRapport(tx: TransactionalSession, id: Rapport.Id): Rapport? =
        queryOf(
                """
                SELECT id, bestilling_id, orgnr, type, dato_valutert, bankkonto, navn, antall_rader, antall_underenheter, antall_personer, opprettet, arkivert
