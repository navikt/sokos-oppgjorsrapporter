...
class VarselRepository {
    ...
    fun finnUprosessertVarsel(tx: TransactionalSession, etter: Instant): Varsel? =
        queryOf(
                ...
                WHERE oppgitt IS NULL AND neste_forsok <= :etter
    ...
    fun oppdater(tx: TransactionalSession, varsel: Varsel) =
        queryOf(
                ...
                UPDATE rapport.varsel_behov
                  SET antall_forsok = :antall_forsok, neste_forsok = :neste_forsok, oppgitt = :oppgitt
                WHERE id = :id
                """
                    .trimIndent(),
                mapOf(
                    "id" to varsel.id.raw,
                    "antall_forsok" to varsel.antallForsok,
                    "neste_forsok" to varsel.nesteForsok,
                    "oppgitt" to varsel.oppgitt,
                ),
            )
            .asUpdate
            .let { tx.run(it) }
        ...
        queryOf(
                ...
                WITH dimensjoner AS (SELECT rapport_type, system, har_feilet
                                     FROM unnest(enum_range(null::rapport.rapport_type)) AS rapport_type,
                                          ...
                                          (VALUES ('false'), ('true'), ('oppgitt')) AS t1(har_feilet)),
                     uferdig AS (SELECT r.id                  AS rapport_id,
                                        r.type                AS rapport_type,
                                        v.system              AS system,
                                        CASE
                                            WHEN v.oppgitt IS NOT NULL THEN 'oppgitt'
                                            WHEN v.antall_forsok >= 1 THEN 'true'
                                            ELSE 'false'
                                        END                   AS har_feilet
                                 FROM rapport.varsel_behov v
            ...
            .map {
                Pair(
                    Tags.of(
                        ...
                        Tag.of("feilet", it.string("har_feilet")),
                    ),
                    it.long("antall"),
                )
            }
